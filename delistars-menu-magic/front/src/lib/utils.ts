...
export function optimizeImage(url: string, width: number = 800, height: number = 600, gravity: string = "g_auto"): string {
  if (!url) return url;
  ...
  if (url.includes('cloudinary.com') && url.includes('/upload/')) {
    ...
    if (parts.length === 2) {
      ...
      return `${parts[0]}/upload/c_fill,${gravity},w_${width},h_${height},q_auto,f_auto/${parts[1]}`;
    }
  }
