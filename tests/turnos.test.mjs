...
const enSede = (schedule, dia, sede) =>
  REGULARES.filter((id) => schedule[id]?.[dia] === sede)
...
test('los descansos son solo de lunes a jueves, uno por día', () => {
  porCadaSemana(({ schedule }) => {
    DIAS.forEach((dia) => {
      const descansan = REGULARES.filter((id) => schedule[id][dia] === DESCANSO)
      assert.equal(descansan.length, esFinde(dia) ? 0 : 1, `descansos el ${dia}`)
    })
  })
})
  ...
  porCadaSemana(({ schedule, caja }) => {
    DIAS.forEach((dia) => {
      ;[SANTA_LUCIA, SANTA_TERESITA].forEach((sede) => {
        ...
        assert.ok(REGLAS.cajeros.includes(cajero), `${cajero} no es cajero`)
...
test('con 6 personas y 4 descansos, dos trabajan la semana completa', () => {
  porCadaSemana(({ schedule }) => {
    const sinDescanso = REGULARES.filter((id) => !DIAS.some((d) => schedule[id][d] === DESCANSO))
    assert.equal(sinDescanso.length, 2)
  })
})
...
test('si alguien sale del equipo el cuadro sigue saliendo', () => {
  ...
  assert.deepEqual(avisos, [])
  DIAS.forEach((dia) => {
    const teresita = ['joseluis', 'sara', 'valentina', 'gendelson']
